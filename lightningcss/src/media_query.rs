    ...
    pub fn parse<'t>(
        ...
        loop {
            match input.parse_until_before(Delimiter::Comma, |i| {
                MediaQuery::parse_with_options(i, options)
            }) {
                Ok(mq) => {
        ...
        D: serde::Deserializer<'de>,
    {
        ...
        match mq {
            ...
            MediaQueryOrRaw::Raw { raw } => {
                let res =
                    MediaQuery::parse_string_with_options(raw.as_ref(), ParserOptions::default())
...
pub enum MediaCondition<'i> {
    ...
    #[cfg_attr(
        feature = "serde",
        serde(borrow, with = "ValueWrapper::<MediaFeature>")
    )]
        ...
        input
            ...
            .or_else(|e| {
                if options.error_recovery {
                    options.warn(e);
                    Ok(MediaCondition::Unknown(TokenList::parse(
                        input, options, 0,
                    )?))
                } else {
                    Err(e)
...
pub(crate) fn parse_query_condition<'t, 'i, P: QueryCondition<'i>>(
    ...
    let (is_negation, function) = match *input.next()? {
        ...
        Token::Function(ref f)
            if flags.contains(QueryConditionFlags::ALLOW_STYLE)
                && f.eq_ignore_ascii_case("style") =>
        {
            (false, QueryFunction::Style)
        }
        Token::Function(ref f)
            if flags.contains(QueryConditionFlags::ALLOW_SCROLL_STATE)
                && f.eq_ignore_ascii_case("scroll-state") =>
        {
...
fn parse_paren_block<'t, 'i, P: QueryCondition<'i>>(
    ...
    input.parse_nested_block(|input| {
        ...
        if let Ok(inner) = input
            .try_parse(|i| parse_query_condition(i, flags | QueryConditionFlags::ALLOW_OR, options))
        {
            return Ok(inner);
        }
    ...
    W: std::fmt::Write,
{
    ...
    to_css_with_parens_if_needed(
        first,
        dest,
        first.needs_parens(Some(operator), &dest.targets.current),
    )?;
    for item in iter {
        dest.write_char(' ')?;
        ...
        to_css_with_parens_if_needed(
            item,
            dest,
            item.needs_parens(Some(operator), &dest.targets.current),
        )?;
    }
            ...
            MediaCondition::Not(ref c) => {
                if let Some(negated) = c.negate() {
                    ...
                    to_css_with_parens_if_needed(
                        &**c,
                        dest,
                        c.needs_parens(None, &dest.targets.current),
                    )
                }
            }
...
impl<'i> MediaFeatureValue<'i> {
    ...
    fn parse_known<'t>(
        ...
        match expected_type {
            ...
            MediaFeatureType::Resolution => {
                Ok(MediaFeatureValue::Resolution(Resolution::parse(input)?))
            }
    ...
    fn parse_unknown<'t>(
        ...
        if let Ok(env) =
            input.try_parse(|input| EnvironmentVariable::parse(input, &ParserOptions::default(), 0))
        {
...
fn process_condition<'i>(
    ...
    match condition {
        MediaCondition::Not(cond) => {
            let used =
                process_condition(loc, custom_media, media_type, qualifier, &mut *cond, seen)?;
            if !used {
        ...
        MediaCondition::Operation { conditions, .. } => {
            ...
            conditions.retain_mut(|condition| {
                let r =
                    process_condition(loc, custom_media, media_type, qualifier, condition, seen);
                if let Ok(used) = r {
                    used
                } else {
                    res = r;
...
mod tests {
    ...
    fn test_and() {
        assert_eq!(
            and("(min-width: 250px)", "(color)"),
            "(width >= 250px) and (color)"
        );
        assert_eq!(
        ...
        assert_eq!(
            and("print", "(min-width: 250px)"),
            "print and (width >= 250px)"
        );
        assert_eq!(
            and("(min-width: 250px)", "print"),
            "print and (width >= 250px)"
        );
        assert_eq!(
