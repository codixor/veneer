    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            UnknownPlaceholder(p, i) => write!(
                f,
                "Error parsing CSS modules pattern: unknown placeholder \"{p}\" at index {i}"
            ),
            UnclosedBrackets(i) => write!(
                f,
                "Error parsing CSS modules pattern: unclosed brackets at index {i}"
            ),
        }
    }
}
...
impl<'i> Pattern<'i> {
            ...
            if input.starts_with('[') {
                if let Some(end_idx) = input.find(']') {
                    let segment = match &input[0..=end_idx] {
                        ...
                        s => {
                            return Err(PatternParseError::UnknownPlaceholder(s.into(), start_idx));
                        }
                    };
    ...
    pub fn has_content_hash(&self) -> bool {
        self.segments
            .iter()
            .any(|s| matches!(s, Segment::ContentHash))
    }
...
impl<'a, 'b, 'c> CssModule<'a, 'b, 'c> {
    ...
    pub fn reference_dashed(
        ...
        let hash = hash(
            &format!("{}_{name}_{key}", self.hashes[source_index as usize]),
            false,
        );
        ...
        for sel in &selectors.0 {
            if sel.len() == 1 {
                match sel.iter_raw_match_order().next().unwrap() {
                    parcel_selectors::parser::Component::Class(id) => {
                        for name in &composes.names {
                            let reference = match &composes.from {
                                ...
                                Some(Specifier::SourceIndex(dep_source_index)) => {
                                    if let Some(entry) = self.exports_by_source_index
                                        [*dep_source_index as usize]
                                        .get(&name.0.as_ref().to_owned())
                                    {
                                        let name = entry.name.clone();
                                        let composes = entry.composes.clone();
                                        let export = self.exports_by_source_index
