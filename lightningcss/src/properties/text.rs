...
impl<'a> schemars::JsonSchema for TextTransformOther {
    ...
    fn json_schema(r#gen: &mut schemars::r#gen::SchemaGenerator) -> schemars::schema::Schema {
        SerializedTextTransformOther::json_schema(r#gen)
    }
...
impl<'i> Parse<'i> for TextIndent {
    fn parse<'t>(input: &mut Parser<'i, 't>) -> Result<Self, ParseError<'i, ParserError<'i>>> {
        ...
        loop {
            ...
            if !hanging {
                if input
                    .try_parse(|input| input.expect_ident_matching("hanging"))
                    .is_ok()
                {
                    hanging = true;
                    continue;
            ...
            if !each_line {
                if input
                    .try_parse(|input| input.expect_ident_matching("each-line"))
                    .is_ok()
                {
                    each_line = true;
                    continue;
...
impl<'a> schemars::JsonSchema for TextDecorationLine {
    ...
    fn json_schema(r#gen: &mut schemars::r#gen::SchemaGenerator) -> schemars::schema::Schema {
        SerializedTextDecorationLine::json_schema(r#gen)
    }
                ...
                macro_rules! logical {
                    ($ltr: ident, $rtl: ident) => {{
                        let logical_supported =
                            !context.should_compile_logical(compat::Feature::LogicalTextAlign);
    ...
    fn finalize(
        ...
        ) = (&mut line, &mut thickness, &mut style, &mut color)
        {
            ...
            if !intersection.is_empty() {
                ...
                let supports_thickness = context
                    .targets
                    .is_compatible(compat::Feature::TextDecorationThicknessShorthand);
                        ...
                        || *color != CssColor::current_color())
                {
                    prefix = context
                        .targets
                        .prefixes(VendorPrefix::None, Feature::TextDecoration);
            ...
            match thickness {
                    ...
                    if should_compile!(context.targets, TextDecorationThicknessPercent) =>
                {
                    let calc = Calc::Function(Box::new(MathFunction::Calc(Calc::Product(
                        p.0,
                        Box::new(Calc::Value(Box::new(LengthPercentage::Dimension(
                            LengthValue::Em(1.0),
                        )))),
                    ))));
                    let thickness = TextDecorationThickness::LengthPercentage(
        ...
        if let (Some((style, style_vp)), Some((color, color_vp))) =
            (&mut emphasis_style, &mut emphasis_color)
        {
            ...
            if !intersection.is_empty() {
                let prefix = context
                    .targets
                    .prefixes(intersection, Feature::TextEmphasis);
