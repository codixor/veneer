...
impl<'i> Parse<'i> for DisplayPair {
    fn parse<'t>(input: &mut Parser<'i, 't>) -> Result<Self, ParseError<'i, ParserError<'i>>> {
        ...
        loop {
            if input
                .try_parse(|input| input.expect_ident_matching("list-item"))
                .is_ok()
            {
                list_item = true;
                continue;
                ...
                (Some(Display::Pair(cur)), Display::Pair(new)) => {
                        ...
                        && cur.inside.is_equivalent(&new.inside)
                    {
                        ...
                        if context.targets.browsers.is_some()
                            && new.inside == DisplayInside::Flex(VendorPrefix::None)
                        {
                            self.decls.clear();
                        } else if context.targets.browsers.is_none()
                            || cur.inside != DisplayInside::Flex(VendorPrefix::None)
                        {
                            self.decls
                                .push(Property::Display(self.display.clone().unwrap()));
                        }
                    }
                }
                _ => {}
        ...
        if let Some(display) = std::mem::take(&mut self.display) {
            ...
            }) = display
            {
                let prefixes = context
                    .targets
                    .prefixes(VendorPrefix::None, Feature::DisplayFlex);
