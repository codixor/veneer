...
impl<'i> PropertyHandler<'i> for TransitionHandler<'i> {
    fn handle_property(
        ...
        match property {
            ...
            TransitionTimingFunction(val, vp) => {
                property!(TransitionTimingFunction, timing_functions, val, vp)
            }
            Transition(val, vp) => {
                let properties: SmallVec<[PropertyId; 1]> =
                    merge_properties(val.iter().map(|b| &b.property));
                maybe_flush!(properties, &properties, vp);

                let durations: SmallVec<[Time; 1]> =
                    val.iter().map(|b| b.duration.clone()).collect();
...
fn merge_properties<'i: 'a, 'a>(
    val: impl Iterator<Item = &'a PropertyId<'i>>,
) -> SmallVec<[PropertyId<'i>; 1]> {
    let mut merged_values =
        SmallVec::<[PropertyId<'_>; 1]>::with_capacity(val.size_hint().1.unwrap_or(1));
    for p in val {
