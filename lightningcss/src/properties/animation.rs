...
impl ToCss for ViewTimeline {
        ...
        W: std::fmt::Write,
    {
        ...
        if self.inset.0 != LengthPercentageOrAuto::Auto
            || self.inset.1 != LengthPercentageOrAuto::Auto
        {
            if needs_space {
                dest.write_char(' ')?;
            }
        ...
        W: std::fmt::Write,
    {
        ...
        let omit_end = match (&self.start.0, &self.end.0) {
            (
                ...
                start_name == end_name
                    && *end_offset == LengthPercentage::Percentage(Percentage(1.0))
            }
            ...
            AnimationName::Ident(CustomIdent(name)) | AnimationName::String(CSSString(name)) => {
                ...
                if self.fill_mode != AnimationFillMode::default()
                    || (!name.eq_ignore_ascii_case("none")
                        && AnimationFillMode::parse_string(&name).is_ok())
                {
                        ...
                        match token {
                            TokenOrValue::Token(Token::Ident(id)) => {
                                if AnimationDirection::parse_string(&id).is_err()
                                    ...
                                    && id.as_ref() != "auto"
                                {
                                    *token = TokenOrValue::AnimationName(AnimationName::Ident(
                                        CustomIdent(id.clone()),
                                    ));
                                }
                            }
                            TokenOrValue::Token(Token::String(s)) => {
                                *token = TokenOrValue::AnimationName(AnimationName::String(
                                    CSSString(s.clone()),
                                ));
                            }
                            _ => {}
    ...
    fn flush(
        ...
        if let (
            ...
            let mut timelines = if let Some(timelines) = &mut timelines_value {
                ...
                Cow::Owned(
                    std::iter::repeat(AnimationTimeline::Auto)
                        .take(len)
                        .collect(),
                )
            } else {
