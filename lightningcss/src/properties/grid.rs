...
impl<'i> TrackList<'i> {
    fn is_explicit(&self) -> bool {
        self.items
            .iter()
            .all(|item| matches!(item, TrackListItem::TrackSize(_)))
    }
}
...
impl<'i> Parse<'i> for GridTemplateAreas {
    fn parse<'t>(input: &mut Parser<'i, 't>) -> Result<Self, ParseError<'i, ParserError<'i>>> {
        ...
        let mut columns = 0;
        while let Ok(s) =
            input.try_parse(|input| input.expect_string().map(|s| s.as_ref().to_owned()))
        {
...
static HTML_SPACE_CHARACTERS: &'static [char] =
...
impl<'a> schemars::JsonSchema for GridAutoFlow {
    ...
    fn json_schema(r#gen: &mut schemars::r#gen::SchemaGenerator) -> schemars::schema::Schema {
        SerializedGridAutoFlow::json_schema(r#gen)
    }
...
impl ToCss for GridAutoFlow {
        ...
        W: std::fmt::Write,
    {
        let s = if *self == GridAutoFlow::Row {
            ...
            if dest.minify { "dense" } else { "row dense" }
...
impl ToCss for GridArea<'_> {
        ...
        W: std::fmt::Write,
    {
        ...
        let can_omit_column_start =
            can_omit_row_end && self.row_start.can_omit_end(&self.column_start);
                ...
                if Grid::is_valid(
                    ...
                    let needs_separate_areas = *areas_val != GridTemplateAreas::None
                        && ((*rows_val == TrackSizing::None
                            && auto_flow_val.direction() == GridAutoFlow::Row)
                            || (*columns_val == TrackSizing::None
                                && auto_flow_val.direction() == GridAutoFlow::Column));
                    ...
                    let (areas_for_grid, areas_for_output) = if needs_separate_areas {
                        ...
                        (
                            GridHandler::pad_grid_template_areas(rows_val, areas_val.clone()),
                            None,
                        )
                    };
