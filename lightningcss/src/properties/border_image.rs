...
impl<'i> BorderImageHandler<'i> {
    ...
    pub fn will_flush(&self, property: &Property<'i>) -> bool {
        use Property::*;
        match property {
            BorderImageSource(_) | BorderImageSlice(_) | BorderImageWidth(_)
            | BorderImageOutset(_) | BorderImageRepeat(_) => {
                self.vendor_prefix != VendorPrefix::None
            }
    ...
    fn flush(
            ...
            && repeat.is_some()
        {
            ...
            if prefix.contains(VendorPrefix::None) && !border_image.slice.fill {
                prefix = context
                    .targets
                    .prefixes(self.vendor_prefix, Feature::BorderImage);
                if !self
                    .flushed_properties
                    .intersects(BorderImageProperty::BorderImage)
                {
            ...
            self.flushed_properties
                .insert(BorderImageProperty::BorderImage);
        } else {
            if let Some(mut source) = source {
                if !self
                    .flushed_properties
                    .contains(BorderImageProperty::BorderImageSource)
                {
