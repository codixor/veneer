...
use super::align::{
    AlignContent, AlignItems, AlignSelf, ContentDistribution, ContentPosition, JustifyContent,
    SelfPosition,
};
...
impl<'i> Parse<'i> for Flex {
    fn parse<'t>(input: &mut Parser<'i, 't>) -> Result<Self, ParseError<'i, ParserError<'i>>> {
        ...
        Ok(Flex {
            ...
            basis: basis.unwrap_or(LengthPercentageOrAuto::LengthPercentage(
...
impl FlexHandler {
    fn flush<'i>(
      ...
      ($prop: ident, $key: ident $(, 2012: $prop_2012: ident )? $(, 2009: $prop_2009: ident )?) => {
        ...
        if let Some((direction, _)) = direction {
            if let Some(targets) = context.targets.browsers {
                let prefixes = context
                    .targets
                    .prefixes(VendorPrefix::None, Feature::FlexDirection);
                let mut prefixes_2009 = VendorPrefix::empty();
                if is_flex_2009(targets) {
                    prefixes_2009 |= VendorPrefix::WebKit;
                }
        ...
        if let (Some((direction, dir_prefix)), Some((wrap, wrap_prefix))) =
        ...
        if let Some(targets) = context.targets.browsers {
            if let Some((grow, _)) = grow {
                let prefixes = context
                    .targets
                    .prefixes(VendorPrefix::None, Feature::FlexGrow);
                let mut prefixes_2009 = VendorPrefix::empty();
                if is_flex_2009(targets) {
                    prefixes_2009 |= VendorPrefix::WebKit;
                }
        ...
        if let (
            Some((grow, grow_prefix)),
            Some((shrink, shrink_prefix)),
            Some((basis, basis_prefix)),
