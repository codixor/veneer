...
impl Matrix3d<f32> {
    ...
    pub fn decompose(&self) -> Option<TransformList> {
        ...
        let row0len =
            (row[0][0] * row[0][0] + row[0][1] * row[0][1] + row[0][2] * row[0][2]).sqrt();
        ...
        let row1len =
            (row[1][0] * row[1][0] + row[1][1] * row[1][1] + row[1][2] * row[1][2]).sqrt();
        ...
        let row2len =
            (row[2][0] * row[2][0] + row[2][1] * row[2][1] + row[2][2] * row[2][2]).sqrt();
        ...
        if a != 0.0 {
            transforms.push(Transform::Rotate3d(
                rotate_x,
                rotate_y,
                rotate_z,
                Angle::Rad(a),
    ...
    pub fn to_matrix(&self) -> Option<Matrix3d<f32>> {
        ...
        match &self {
            ...
            Transform::RotateX(angle) => {
                return Some(Matrix3d::rotate(1.0, 0.0, 0.0, to_radians!(angle)));
            }
            Transform::RotateY(angle) => {
                return Some(Matrix3d::rotate(0.0, 1.0, 0.0, to_radians!(angle)));
            }
            Transform::Rotate3d(x, y, z, angle) => {
                return Some(Matrix3d::rotate(*x, *y, *z, to_radians!(angle)));
            }
