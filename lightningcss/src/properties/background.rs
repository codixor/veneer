...
impl<'i> ToCss for Background<'i> {
        ...
        W: std::fmt::Write,
    {
        ...
        if (output_padding_box && self.clip != self.origin)
            || self.clip != BackgroundOrigin::BorderBox
        {
            if has_output {
                dest.write_str(" ")?;
            }
...
impl<'i> Shorthand<'i> for SmallVec<[Background<'i>; 1]> {
    fn from_longhands(
            ...
            && clips.is_some()
        {
            ...
            let mut x_positions = x_positions.unwrap();
            let mut y_positions = y_positions.unwrap();
            let mut repeats = repeats.unwrap();
            let mut sizes = sizes.unwrap();
            let mut attachments = attachments.unwrap();
            let mut origins = origins.unwrap();
            ...
            .enumerate()
            .map(
                |(i, (image, x_position, y_position, repeat, size, attachment, origin, clip))| {
                    Background {
                        color: if i == length - 1 {
                            color.clone().unwrap()
                        } else {
    ...
    fn longhand(&self, property_id: &PropertyId) -> Option<Property<'i>> {
        match property_id {
            PropertyId::BackgroundColor => Some(Property::BackgroundColor(
                self.last().unwrap().color.clone(),
            )),
            ...
            PropertyId::BackgroundSize => Some(Property::BackgroundSize(
                self.iter().map(|v| v.size.clone()).collect(),
            )),
...
impl<'i> BackgroundHandler<'i> {
    fn flush(
        ...
        if let (
                ...
                && clips.0.len() == len
            {
                ...
                .map(
                    |(
                        i,
                        (image, x_position, y_position, repeat, size, attachment, origin, clip),
                    )| Background {
                        color: if i == len - 1 {
                            color.clone()
                        } else {
                ...
                if !self
                    .flushed_properties
                    .intersects(BackgroundProperty::Background)
                {
                ...
                if let Some(clip) = clip_property {
                    dest.push(clip);
                    self.flushed_properties
                        .insert(BackgroundProperty::BackgroundClip);
                }
        ...
        if let Some(mut color) = color {
            if !self
                .flushed_properties
                .contains(BackgroundProperty::BackgroundColor)
            {
        ...
        if let Some(mut images) = images {
            if !self
                .flushed_properties
                .contains(BackgroundProperty::BackgroundImage)
            {
