...
use crate::targets::{Features, Targets, should_compile};
use crate::traits::{Parse, ParseWithOptions, ToCss};
use crate::values::angle::Angle;
use crate::values::color::{
    ColorFallbackKind, ComponentParser, CssColor, HSL, LightDarkColor, RGB, RGBA,
    parse_hsl_hwb_components, parse_rgb_components,
};
use crate::values::ident::{CustomIdent, DashedIdent, DashedIdentReference, Ident};
use crate::values::length::{LengthValue, serialize_dimension};
            ...
            match input.next_including_whitespace_and_comments() {
                ...
                | Ok(token @ &cssparser::Token::CurlyBracketBlock) => {
                    ...
                    input.parse_nested_block(|input| {
                        TokenList::parse_raw(input, tokens, options, depth + 1)
                    })?;
            ...
            match input.next_including_whitespace_and_comments() {
                Ok(&cssparser::Token::Function(ref f)) => {
                    ...
                    if let Some(color) = try_parse_color_token(&f, &state, input) {
                        ...
                        input.try_parse(|input| UnresolvedColor::parse(&f, input, options))
                    {
                        ...
                        let arguments = input.parse_nested_block(|input| {
                            TokenList::parse(input, options, depth + 1)
                        })?;
                ...
                | Ok(token @ &cssparser::Token::CurlyBracketBlock) => {
                    ...
                    input.parse_nested_block(|input| {
                        TokenList::parse_into(input, tokens, options, depth + 1)
                    })?;
...
impl<'i> TokenList<'i> {
    ...
    pub(crate) fn starts_with_whitespace(&self) -> bool {
        matches!(
            self.0.get(0),
            Some(TokenOrValue::Token(Token::WhiteSpace(_)))
        )
    }
}
...
pub enum Token<'a> {
    ...
    #[cfg_attr(
        feature = "serde",
        serde(rename = "id-hash", with = "ValueWrapper::<CowArcStr>")
    )]
...
impl<'a> ToCss for Token<'a> {
        ...
        W: std::fmt::Write,
    {
        ...
        match self {
            ...
            Token::UnquotedUrl(x) => {
                cssparser::Token::UnquotedUrl(x.as_ref().into()).to_css(dest)?
            }
        ...
        if !lowest_fallback.is_empty() {
            for token in self.0.iter_mut() {
                match token {
                    ...
                    TokenOrValue::Var(v) if v.fallback.is_some() => {
                        *v = v.get_fallback(lowest_fallback)
                    }
                    TokenOrValue::Env(v) if v.fallback.is_some() => {
                        *v = v.get_fallback(lowest_fallback)
                    }
                    _ => {}
                }
    ...
    fn get_fallback(&self, kind: ColorFallbackKind) -> Self {
        Variable {
            ...
            fallback: self
                .fallback
                .as_ref()
                .map(|fallback| fallback.get_fallback(kind)),
        }
    }
}
...
pub enum EnvironmentVariableName<'i> {
    ...
    #[cfg_attr(
        feature = "serde",
        serde(with = "crate::serialization::ValueWrapper::<CustomIdent>")
    )]
...
impl<'i> Parse<'i> for EnvironmentVariableName<'i> {
    fn parse<'t>(input: &mut Parser<'i, 't>) -> Result<Self, ParseError<'i, ParserError<'i>>> {
        ...
        if let Ok(dashed) = input.try_parse(|input| {
            DashedIdentReference::parse_with_options(input, &ParserOptions::default())
        }) {
