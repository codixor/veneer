    ...
    fn is_compatible(&self, browsers: Browsers) -> bool {
        match self {
            ...
            ListStyleType::String(..) => {
                crate::compat::Feature::StringListStyleType.is_compatible(browsers)
            }
...
impl<'i> Parse<'i> for CounterStyle<'i> {
    fn parse<'t>(input: &mut Parser<'i, 't>) -> Result<Self, ParseError<'i, ParserError<'i>>> {
        ...
        if input
            .try_parse(|input| input.expect_function_matching("symbols"))
            .is_ok()
        {
    ...
    fn is_compatible(&self, browsers: Browsers) -> bool {
        match self {
            ...
            CounterStyle::Symbols { .. } => {
                crate::compat::Feature::SymbolsListStyleType.is_compatible(browsers)
            }
        }
    }
}
