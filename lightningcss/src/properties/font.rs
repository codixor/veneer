...
impl IsCompatible for GenericFontFamily {
    fn is_compatible(&self, browsers: crate::targets::Browsers) -> bool {
        ...
        match self {
            ...
            UISerif | UISansSerif | UIMonospace | UIRounded => {
                Feature::ExtendedSystemFonts.is_compatible(browsers)
            }
            _ => true,
        }
    }
...
pub enum FontStyle {
    ...
    Oblique(
        #[cfg_attr(feature = "serde", serde(default = "FontStyle::default_oblique_angle"))] Angle,
    ),
}
    ...
    fn flush(
        ...
        if !self.flushed_properties.contains(FontProperty::FontFamily) {
            family = compatible_font_family(
                family,
                !should_compile!(context.targets, FontFamilySystemUi),
            );
        }
...
fn compatible_font_family(
    mut family: Option<Vec<FontFamily>>,
    is_supported: bool,
