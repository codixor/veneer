...
pub enum ClipPath<'i> {
    ...
    #[cfg_attr(
        feature = "serde",
        serde(borrow, with = "crate::serialization::ValueWrapper::<Url>")
    )]
    Url(Url<'i>),
    ...
    #[cfg_attr(
        feature = "serde",
        serde(with = "crate::serialization::ValueWrapper::<GeometryBox>")
    )]
    Box(GeometryBox),
}
    ...
    fn handle_property(
      ...
      ($prop: ident, $val: expr, $vp: expr) => {{
        ...
        match property {
            ...
            Property::WebKitMaskBoxImageSource(val, _) => {
                property!(border_source, val, &VendorPrefix::WebKit)
            }
            ...
            Property::WebKitMaskBoxImageSlice(val, _) => {
                property!(border_slice, val, &VendorPrefix::WebKit)
            }
            Property::MaskBorderWidth(val) => property!(border_width, val, &VendorPrefix::None),
            Property::WebKitMaskBoxImageWidth(val, _) => {
                property!(border_width, val, &VendorPrefix::WebKit)
            }
            Property::MaskBorderOutset(val) => property!(border_outset, val, &VendorPrefix::None),
            Property::WebKitMaskBoxImageOutset(val, _) => {
                property!(border_outset, val, &VendorPrefix::WebKit)
            }
            Property::MaskBorderRepeat(val) => property!(border_repeat, val, &VendorPrefix::None),
            Property::WebKitMaskBoxImageRepeat(val, _) => {
                property!(border_repeat, val, &VendorPrefix::WebKit)
            }
...
impl<'i> MaskHandler<'i> {
    ...
    fn flush_mask(
        ...
        if let (
            ...
            let len = images.len();
            let intersection =
                *images_vp & *positions_vp & *sizes_vp & *repeats_vp & *clips_vp & *origins_vp;
                ...
                && mode_vals.len() == len
            {
                ...
                .map(
                    |(image, position, size, repeat, clip, origin, composite, mode)| Mask {
                        image,
                        position,
                        size,
                        repeat,
                ...
                if !self.flushed_properties.intersects(MaskProperty::Mask) {
                    for fallback in masks.get_fallbacks(context.targets) {
                        ...
                        let mut p = fallback.iter().fold(VendorPrefix::empty(), |p, mask| {
                            p | mask.image.get_vendor_prefix()
                        }) - VendorPrefix::None
                            & prefix;
                        if p.is_empty() {
                            p = prefix;
                    ...
                    let p = masks.iter().fold(VendorPrefix::empty(), |p, mask| {
                        p | mask.image.get_vendor_prefix()
                    }) - VendorPrefix::None
                        & prefix;
                    if !p.is_empty() {
            ...
            if !vp.is_empty() {
                ...
                if !self.flushed_properties.contains(MaskProperty::MaskImage) {
                    ...
                    for fallback in images.get_fallbacks(context.targets) {
                        ...
                        // can only be used in -webkit-mask-image.
                        // However, if mask-image is unprefixed, gradients can still be.
                        let mut p = fallback.iter().fold(VendorPrefix::empty(), |p, image| {
                            p | image.get_vendor_prefix()
                        }) - VendorPrefix::None
                            & prefix;
                        if p.is_empty() {
                            p = prefix;
                    ...
                    let p = images.iter().fold(VendorPrefix::empty(), |p, image| {
                        p | image.get_vendor_prefix()
                    }) - VendorPrefix::None
                        & prefix;
                    if !p.is_empty() {
        ...
        if let Some(composites) = composites {
            let prefix = context
                .targets
                .prefixes(VendorPrefix::None, Feature::MaskComposite);
            if prefix.contains(VendorPrefix::WebKit) {
    ...
    fn flush_mask_shorthand(
        ...
        if prefix.contains(VendorPrefix::WebKit)
            && masks.iter().any(|mask| {
                mask.composite != MaskComposite::default() || mask.mode != MaskMode::default()
            })
        {
    ...
    fn flush_mask_border(
        ...
        if let (
            ...
            if !intersection.is_empty()
                && (!intersection.contains(VendorPrefix::None) || mode.is_some())
            {
                ...
                if !self.flushed_properties.intersects(MaskProperty::MaskBorder) {
                    ...
                    for fallback in fallbacks {
                        let mut p =
                            fallback.source.get_vendor_prefix() - VendorPrefix::None & prefix;
                        if p.is_empty() {
                            p = prefix;
                        }
        ...
        if let Some((mut source, mut prefix)) = source {
            ...
            if !self
                .flushed_properties
                .contains(MaskProperty::MaskBorderSource)
            {
                // Get vendor prefix and color fallbacks.
            ...
            if prefix.contains(VendorPrefix::None) {
                ...
                self.flushed_properties
                    .insert(MaskProperty::MaskBorderSource);
            }
        }
