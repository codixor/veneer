...
impl<'i, T: Clone> SupportsRule<'i, T> {
    pub(crate) fn minify(
        ...
        let inserted = context
            .targets
            .enter_supports(self.condition.get_supported_features());
        if inserted {
            context.handler_context.targets = context.targets.current;
        }

        self.condition
            .set_prefixes_for_targets(&context.targets.current);
...
pub enum SupportsCondition<'i> {
    ...
    #[cfg_attr(
        feature = "serde",
        serde(with = "ValueWrapper::<Box<SupportsCondition>>")
    )]
    ...
    #[cfg_attr(
        feature = "serde",
        serde(with = "ValueWrapper::<Vec<SupportsCondition>>")
    )]
...
impl<'i> SupportsCondition<'i> {
    ...
    fn get_supported_features(&self) -> Features {
        fn get_supported_features_internal(value: &SupportsCondition) -> Option<Features> {
            match value {
                SupportsCondition::And(list) => list
                    .iter()
                    .map(|c| get_supported_features_internal(c))
                    .try_union_all(),
            ...
            let condition = input.try_parse(|input| {
                ...
                if let Some(expected) = expected_type {
                    if found_type != expected {
                        return Err(
                            location.new_unexpected_token_error(cssparser::Token::Ident(s.clone()))
                        );
                    }
                } else {
            ...
            if let Ok(condition) = condition {
                if conditions.is_empty() {
                    ...
                    if let SupportsCondition::Declaration { property_id, value } = &in_parens {
                        seen_declarations.insert(
                            (property_id.with_prefix(VendorPrefix::None), value.clone()),
                            0,
                        );
                    }
                }
...
impl<'i> SupportsCondition<'i> {
    fn parse_in_parens<'t>(
        ...
        input
            .parse_nested_block(|input| input.expect_no_error_token().map_err(|err| err.into()))?;
    ...
    fn to_css_with_parens_if_needed<W>(
        &self,
        dest: &mut Printer<W>,
        needs_parens: bool,
    ) -> Result<(), PrinterError>
