...
use crate::selector::{
    SelectorList, downlevel_selectors, get_prefix, is_compatible, is_pure_css_modules_selector,
    is_unused,
};
...
impl<'i, T> StyleRule<'i, T> {
    ...
    pub(crate) fn update_prefix(&mut self, context: &mut MinifyContext<'_, 'i>) {
        ...
        {
            self.vendor_prefix =
                downlevel_selectors(self.selectors.0.as_mut_slice(), context.targets.current);
        }
    }
}
...
impl<'a, 'i, T: ToCss> StyleRule<'i, T> {
        ...
        W: std::fmt::Write,
    {
        ...
        let supports_nesting =
            self.rules.0.is_empty() || !should_compile!(dest.targets.current, Nesting);
        let len =
