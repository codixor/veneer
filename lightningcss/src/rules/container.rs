...
use crate::media_query::{
    FeatureToCss, MediaFeatureType, Operator, QueryCondition, QueryConditionFlags, QueryFeature,
    ValueType, define_query_features, operation_to_css, parse_query_condition,
...
pub enum ContainerCondition<'i> {
    ...
    #[cfg_attr(
        feature = "serde",
        serde(borrow, with = "ValueWrapper::<ContainerSizeFeature>")
    )]
    ...
    #[cfg_attr(
        feature = "serde",
        serde(with = "ValueWrapper::<Box<ContainerCondition>>")
    )]
    ...
    #[cfg_attr(
        feature = "serde",
        serde(borrow, with = "ValueWrapper::<ScrollStateQuery>")
    )]
...
pub enum ScrollStateQuery<'i> {
    ...
    #[cfg_attr(
        feature = "serde",
        serde(borrow, with = "ValueWrapper::<ScrollStateFeature>")
    )]
    ...
    #[cfg_attr(
        feature = "serde",
        serde(with = "ValueWrapper::<Box<ScrollStateQuery>>")
    )]
...
impl<'i> QueryCondition<'i> for ContainerCondition<'i> {
    ...
    fn parse_style_query<'t>(
        ...
        input.parse_nested_block(|input| {
            if let Ok(res) = input.try_parse(|input| {
                parse_query_condition(input, QueryConditionFlags::ALLOW_OR, options)
            }) {
    ...
    fn parse_scroll_state_query<'t>(
        ...
        input.parse_nested_block(|input| {
            ...
            Ok(Self::ScrollState(ScrollStateQuery::parse_feature(
                input, options,
            )?))
        })
    }
        ...
        input
            ...
            .or_else(|e| {
                if options.error_recovery {
                    options.warn(e);
                    Ok(ContainerCondition::Unknown(TokenList::parse(
                        input, options, 0,
                    )?))
                } else {
                    Err(e)
