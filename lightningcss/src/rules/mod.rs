...
use crate::parser::{
    DefaultAtRule, DefaultAtRuleParser, TopLevelRuleParser, parse_rule_list, parse_style_block,
};
use crate::prefixes::Feature;
use crate::printer::Printer;
use crate::rules::keyframes::KeyframesName;
use crate::selector::{Component, Selector, SelectorList, is_compatible, is_equivalent};
use crate::stylesheet::ParserOptions;
use crate::targets::{TargetsWithSupportsScope, should_compile};
...
use cssparser::{ParseError, Parser, ParserInput, parse_one_rule};
...
impl<'i, T> CssRule<'i, T> {
    ...
    pub fn parse_with<'t, P: AtRuleParser<'i, AtRule = T>>(
        ...
        parse_one_rule(
            input,
            &mut TopLevelRuleParser::new(options, at_rule_parser, &mut rules),
        )?;
...
impl<'i, T: Clone> CssRuleList<'i, T> {
    pub(crate) fn minify(
        ...
        let mut style_rules = HashMap::with_capacity_and_hasher(
            self.0.len(),
            BuildHasherDefault::<PrecomputedHasher>::default(),
        );
...
fn merge_style_rules<'i, T>(
        ...
        && last_style_rule.rules.0.is_empty()
    {
        ...
        if style.is_compatible(context.targets.current)
            && last_style_rule.is_compatible(context.targets.current)
        {
            last_style_rule
                .selectors
                .0
                .extend(style.selectors.0.drain(..));
            if style.vendor_prefix.contains(VendorPrefix::None)
                && context.targets.current.should_compile_selectors()
            {
                last_style_rule.vendor_prefix = style.vendor_prefix;
