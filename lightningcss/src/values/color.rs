...
use crate::targets::{Browsers, Features, Targets, should_compile};
...
use cssparser::*;
use cssparser_color::{AngleOrNumber, ColorParser, NumberOrPercentage, hsl_to_rgb};
use std::any::TypeId;
...
impl CssColor {
    ...
    pub fn to_rgb(&self) -> Result<CssColor, ()> {
        match self {
            CssColor::LightDark(light, dark) => Ok(CssColor::LightDark(
                Box::new(light.to_rgb()?),
                Box::new(dark.to_rgb()?),
            )),
    ...
    pub(crate) fn get_possible_fallbacks(&self, targets: Targets) -> ColorFallbackKind {
        ...
        let mut fallbacks = match self {
            ...
            CssColor::Predefined(predefined) => match &**predefined {
                PredefinedColor::DisplayP3(..) if should_compile!(targets, P3Colors) => {
...
impl ToCss for CssColor {
        ...
        W: std::fmt::Write,
    {
        match self {
            ...
            CssColor::LAB(lab) => match &**lab {
                LABColor::LAB(lab) => {
                    write_components("lab", lab.l / 100.0, lab.a, lab.b, lab.alpha, dest)
                }
                LABColor::LCH(lch) => {
                    write_components("lch", lch.l / 100.0, lch.c, lch.h, lch.alpha, dest)
                }
                LABColor::OKLAB(lab) => {
                    write_components("oklab", lab.l, lab.a, lab.b, lab.alpha, dest)
                }
                LABColor::OKLCH(lch) => {
                    write_components("oklch", lch.l, lch.c, lch.h, lch.alpha, dest)
                }
            },
...
impl<'i> ColorParser<'i> for RelativeComponentParser {
    ...
    fn parse_angle_or_number<'t>(
        ...
        if let Ok((value, ty)) = input
            .try_parse(|input| self.parse_ident(input, ChannelType::Angle | ChannelType::Number))
        {
            return Ok(match ty {
        ...
        if let Ok(value) = input.try_parse(
            |input| -> Result<AngleOrNumber, ParseError<'i, ParserError<'i>>> {
                match Calc::parse_with(input, |ident| {
                    self.get_ident(ident, ChannelType::Angle | ChannelType::Number)
    ...
    fn parse_number<'t>(
        ...
        if let Ok((value, _)) =
            input.try_parse(|input| self.parse_ident(input, ChannelType::Number))
        {
            return Ok(value);
        }

        match Calc::parse_with(input, |ident| {
            self.get_ident(ident, ChannelType::Number)
                .map(|(v, _)| Calc::Number(v))
        }) {
    ...
    fn parse_percentage<'t>(
        ...
        if let Ok((value, _)) =
            input.try_parse(|input| self.parse_ident(input, ChannelType::Percentage))
        {
            return Ok(value);
        }

        if let Ok(value) = input.try_parse(
            |input| -> Result<Percentage, ParseError<'i, ParserError<'i>>> {
                match Calc::parse_with(input, |ident| {
                    self.get_ident(ident, ChannelType::Percentage)
    ...
    fn parse_number_or_percentage<'t>(
        ...
        if let Ok((value, ty)) = input.try_parse(|input| {
            self.parse_ident(input, ChannelType::Percentage | ChannelType::Number)
        }) {
            return Ok(match ty {
        ...
        if let Ok(value) = input.try_parse(
            |input| -> Result<NumberOrPercentage, ParseError<'i, ParserError<'i>>> {
                match Calc::parse_with(input, |ident| {
                    self.get_ident(ident, ChannelType::Percentage | ChannelType::Number)
                        .map(|(value, ty)| match ty {
    ...
    input.parse_nested_block(|input| {
        parser.parse_relative::<RGB, _, _>(input, |input, parser| {
            ...
            if !r.is_nan() && !g.is_nan() && !b.is_nan() && !alpha.is_nan() {
                ...
                Ok(CssColor::Float(Box::new(FloatColor::RGB(RGB {
                    r,
                    g,
                    b,
                    alpha,
    ...
    let (r, g, b) = if is_legacy_syntax {
        ...
        fn get_component<'i, 't>(value: NumberOrPercentage) -> f32 {
            match value {
                ...
                NumberOrPercentage::Percentage { unit_value } => {
                    (unit_value * 255.0).round().clamp(0.0, 255.0)
                }
            }
        }
