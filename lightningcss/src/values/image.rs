    ...
    fn is_compatible(&self, browsers: Browsers) -> bool {
        match self {
            Image::Gradient(g) => match &**g {
                Gradient::Linear(g) => {
                    compat::Feature::LinearGradient.is_compatible(browsers)
                        && g.is_compatible(browsers)
                }
                Gradient::RepeatingLinear(g) => {
                    compat::Feature::RepeatingLinearGradient.is_compatible(browsers)
                        && g.is_compatible(browsers)
                }
                Gradient::Radial(g) => {
                    compat::Feature::RadialGradient.is_compatible(browsers)
                        && g.is_compatible(browsers)
                }
                Gradient::RepeatingRadial(g) => {
                    compat::Feature::RepeatingRadialGradient.is_compatible(browsers)
                        && g.is_compatible(browsers)
                }
                Gradient::Conic(g) => {
                    compat::Feature::ConicGradient.is_compatible(browsers)
                        && g.is_compatible(browsers)
                }
                Gradient::RepeatingConic(g) => {
                    compat::Feature::RepeatingConicGradient.is_compatible(browsers)
                        && g.is_compatible(browsers)
                }
...
impl<'i> Parse<'i> for ImageSet<'i> {
    fn parse<'t>(input: &mut Parser<'i, 't>) -> Result<Self, ParseError<'i, ParserError<'i>>> {
        ...
        let options =
            input.parse_nested_block(|input| input.parse_comma_separated(ImageSetOption::parse))?;
        Ok(ImageSet {
            options,
            vendor_prefix,
        })
    ...
    fn is_compatible(&self, browsers: Browsers) -> bool {
        compat::Feature::ImageSet.is_compatible(browsers)
            && self
                .options
                .iter()
                .all(|opt| opt.image.is_compatible(browsers))
    }
}
...
fn parse_file_type<'i, 't>(
    input: &mut Parser<'i, 't>,
) -> Result<CowRcStr<'i>, ParseError<'i, ParserError<'i>>> {
