        ...
        match self {
            ...
            SyntaxString::Components(components) => {
                ...
                for component in components {
                    ...
                    loop {
                        let value: Result<ParsedComponent<'i>, ParseError<'i, ParserError<'i>>> =
                            input.try_parse(|input| {
                                Ok(match &component.kind {
                                    SyntaxComponentKind::Length => ParsedComponent::Length(
                                        values::length::Length::parse(input)?,
                                    ),
                                    SyntaxComponentKind::Number => {
                                        ParsedComponent::Number(CSSNumber::parse(input)?)
                                    }
                                    SyntaxComponentKind::Percentage => ParsedComponent::Percentage(
                                        values::percentage::Percentage::parse(input)?,
                                    ),
                                    SyntaxComponentKind::LengthPercentage => {
                                        ParsedComponent::LengthPercentage(
                                            values::length::LengthPercentage::parse(input)?,
                                        )
                                    }
                                    SyntaxComponentKind::String => ParsedComponent::String(
                                        values::string::CSSString::parse(input)?,
                                    ),
                                    SyntaxComponentKind::Color => ParsedComponent::Color(
                                        values::color::CssColor::parse(input)?,
                                    ),
                                    SyntaxComponentKind::Image => {
                                        ParsedComponent::Image(values::image::Image::parse(input)?)
                                    }
                                    SyntaxComponentKind::Url => {
                                        ParsedComponent::Url(values::url::Url::parse(input)?)
                                    }
                                    SyntaxComponentKind::Integer => {
                                        ParsedComponent::Integer(CSSInteger::parse(input)?)
                                    }
                                    SyntaxComponentKind::Angle => {
                                        ParsedComponent::Angle(values::angle::Angle::parse(input)?)
                                    }
                                    SyntaxComponentKind::Time => {
                                        ParsedComponent::Time(values::time::Time::parse(input)?)
                                    }
                                    SyntaxComponentKind::Resolution => ParsedComponent::Resolution(
                                        values::resolution::Resolution::parse(input)?,
                                    ),
                                    SyntaxComponentKind::TransformFunction => {
                                        ParsedComponent::TransformFunction(
                                            crate::properties::transform::Transform::parse(input)?,
                                        )
                                    }
                                    SyntaxComponentKind::TransformList => {
                                        ParsedComponent::TransformList(
                                            crate::properties::transform::TransformList::parse(
                                                input,
                                            )?,
                                        )
                                    }
                                    SyntaxComponentKind::CustomIdent => {
                                        ParsedComponent::CustomIdent(
                                            values::ident::CustomIdent::parse(input)?,
                                        )
                                    }
    ...
    fn test_syntax() {
        ...
        test(
            ...
            ParsedComponent::Repeated {
                components: vec![ParsedComponent::Color(values::color::CssColor::RGBA(
                    RGBA {
                        red: 255,
                        green: 0,
                        blue: 0,
        ...
        test(
            "<length>",
            "25px",
            ParsedComponent::Length(values::length::Length::Value(
        ...
        test(
            ...
            "calc(25px + 25px)",
            ParsedComponent::Length(values::length::Length::Value(
                values::length::LengthValue::Px(50.0),
            )),
        );
