...
use crate::targets::{Browsers, Targets, should_compile};
...
impl Gradient {
    ...
    pub fn get_necessary_prefixes(&self, targets: Targets) -> VendorPrefix {
        ...
        match self {
            ...
            Gradient::RepeatingLinear(linear) => {
                get_prefixes!(RepeatingLinearGradient, linear.vendor_prefix)
            }
            Gradient::Radial(radial) => get_prefixes!(RadialGradient, radial.vendor_prefix),
            Gradient::RepeatingRadial(radial) => {
                get_prefixes!(RepeatingRadialGradient, radial.vendor_prefix)
            }
            _ => VendorPrefix::None,
        }
    }
    ...
    pub fn get_prefixed(&self, prefix: VendorPrefix) -> Gradient {
        match self {
            Gradient::Linear(linear) => {
                ...
                let needs_legacy_direction =
                    linear.vendor_prefix == VendorPrefix::None && prefix != VendorPrefix::None;
    ...
    pub fn get_legacy_webkit(&self) -> Result<Gradient, ()> {
        Ok(Gradient::WebKitGradient(WebKitGradient::from_standard(
            self,
        )?))
    }
...
impl LinearGradient {
    fn parse<'i, 't>(
        ...
        let direction = if let Ok(direction) = input
            .try_parse(|input| LineDirection::parse(input, vendor_prefix != VendorPrefix::None))
        {
            input.expect_comma()?;
            direction
        } else {
                            ...
                            GradientItem::ColorStop(ColorStop {
                                ...
                                position: position.clone().map(|p| match p {
                                    LengthPercentage::Percentage(p) => {
                                        LengthPercentage::Percentage(Percentage(1.0 - p.0))
                                    }
                                    _ => unreachable!(),
                                }),
                            })
    ...
    fn get_fallback(&self, kind: ColorFallbackKind) -> LinearGradient {
        LinearGradient {
            ...
            items: self
                .items
                .iter()
                .map(|item| item.get_fallback(kind))
                .collect(),
...
pub enum LineDirection {
    ...
    #[cfg_attr(
        feature = "serde",
        serde(with = "ValueWrapper::<HorizontalPositionKeyword>")
    )]
    Horizontal(HorizontalPositionKeyword),
    /// A vertical posision keyword, e.g. `top` or `bottom`.
    #[cfg_attr(
        feature = "serde",
        serde(with = "ValueWrapper::<VerticalPositionKeyword>")
    )]
...
impl<'i> Parse<'i> for Circle {
    fn parse<'t>(input: &mut Parser<'i, 't>) -> Result<Self, ParseError<'i, ParserError<'i>>> {
        ...
        if input
            .try_parse(|input| input.expect_ident_matching("circle"))
            .is_ok()
        {
            if let Ok(extent) = input.try_parse(ShapeExtent::parse) {
                return Ok(Circle::Extent(extent));
...
impl<'i> Parse<'i> for Ellipse {
    fn parse<'t>(input: &mut Parser<'i, 't>) -> Result<Self, ParseError<'i, ParserError<'i>>> {
        ...
        if input
            .try_parse(|input| input.expect_ident_matching("ellipse"))
            .is_ok()
        {
            if let Ok(extent) = input.try_parse(ShapeExtent::parse) {
                return Ok(Ellipse::Extent(extent));
...
impl<D> IsCompatible for GradientItem<D> {
    fn is_compatible(&self, browsers: Browsers) -> bool {
        match self {
            ...
            GradientItem::Hint(..) => {
                compat::Feature::GradientInterpolationHints.is_compatible(browsers)
            }
        }
    }
}
