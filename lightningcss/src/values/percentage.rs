...
use super::angle::{Angle, impl_try_from_angle};
...
use crate::traits::{
    Op, Parse, Sign, ToCss, TryMap, TryOp, TrySign, Zero, impl_op, private::TryAdd,
};
#[cfg(feature = "visitor")]
    ...
    fn add(self, other: DimensionPercentage<D>) -> DimensionPercentage<D> {
        ...
        match res {
            DimensionPercentage::Calc(c) => match *c {
                ...
                c => DimensionPercentage::Calc(Box::new(Calc::Function(Box::new(
                    MathFunction::Calc(c),
                )))),
            },
            _ => res,
...
impl<D: TryAdd<D> + Clone + Zero + TrySign + std::fmt::Debug> AddInternal
    for DimensionPercentage<D>
{
    fn add(self, other: Self) -> Self {
