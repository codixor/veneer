...
use crate::{
    ...
    properties::{
        ...
        custom::{
            CustomProperty, EnvironmentVariableName, TokenList, TokenOrValue, UnparsedProperty,
            UnresolvedColor,
        },
    },
    rules::{
...
impl<'i, T: std::error::Error> std::fmt::Display for BundleErrorKind<'i, T> {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        ...
        match self {
            ...
            UnsupportedMediaBooleanLogic => {
                write!(f, "Unsupported boolean logic in @import media query")
            }
...
impl<'a, 'o, 's, P: SourceProvider, T: AtRuleParser<'a> + Clone + Sync + Send>
    Bundler<'a, 'o, 's, P, T>
where
    T::AtRule: Sync + Send + ToCss + Clone,
{
    ...
    fn load_file(
        ...
        file: &Path,
        rule: ImportRule<'a>,
    ) -> Result<u32, Error<BundleErrorKind<'a, P::Error>>> {
    ...
    fn inline(&mut self, dest: &mut Vec<CssRule<'a, T::AtRule>>) {
        ...
        fn process<'a, T>(
            ...
            for i in 0..stylesheet.css_modules_deps.len() {
                ...
                if resolved.parent_source_index == source_index
            ...
            for rule in &mut rules {
                match rule {
                    CssRule::Import(_) => {
                        let dep_source_index =
                            stylesheets[source_index as usize].dependencies[import_index];
                        let resolved = &stylesheets[dep_source_index as usize];
        ...
        fn resolve(
            ...
            if specifier.starts_with("foo:") {
                ...
                let err = std::io::Error::new(
                    ...
                    format!(
                        "Failed to resolve `{specifier}`, specifier does not start with `foo:`.",
                    ),
                );
            ...
            Some(Box::new(|err| {
                ...
                assert!(
                    err.to_string().contains(
                        "Failed to resolve `/b.css`, specifier does not start with `foo:`."
                    )
                );
            })),
        );
