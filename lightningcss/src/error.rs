    ...
    fn fmt(&self, f: &mut fmt::Formatter<'_>) -> fmt::Result {
        ...
        match self {
            ...
            ClassNeedsIdent(token) => {
                write!(f, "Expected identifier in class selector, got {token:?}")
            }
            ...
            ExplicitNamespaceUnexpectedToken(token) => {
                write!(f, "Unexpected token in namespace selector: {token:?}")
            }
            InvalidPseudoClassAfterPseudoElement => write!(
                f,
                "Invalid pseudo class after pseudo element, only user action pseudo classes (e.g. :hover, :active) are allowed"
            ),
            InvalidPseudoClassAfterWebKitScrollbar => write!(
                f,
                "Invalid pseudo class after ::-webkit-scrollbar pseudo element"
            ),
            InvalidPseudoClassBeforeWebKitScrollbar => write!(
                f,
                "Pseudo class must be prefixed by a ::-webkit-scrollbar pseudo element"
            ),
            InvalidQualNameInAttr(token) => {
                write!(f, "Invalid qualified name in attribute selector: {token:?}")
            }
            MissingNestingPrefix => write!(
                f,
                "A nested rule must start with a nesting selector (&) as prefix of each selector, or start with @nest"
            ),
            MissingNestingSelector => write!(
                f,
                "A nesting selector (&) is required in each selector of a @nest rule"
            ),
            NoQualifiedNameInAttributeSelector(token) => {
                write!(f, "No qualified name in attribute selector: {token:?}.")
            }
            PseudoElementExpectedIdent(token) => {
                write!(f, "Invalid token in pseudo element: {token:?}")
            }
            UnexpectedIdent(name) => write!(f, "Unexpected identifier: {name}"),
            UnexpectedTokenInAttributeSelector(token) => {
                write!(f, "Unexpected token in attribute selector: {token:?}")
            }
            UnsupportedPseudoClass(name) => write!(
                f,
                "'{name}' is not recognized as a valid pseudo-class. Did you mean '::{name}' (pseudo-element) or is this a typo?"
            ),
            UnsupportedPseudoElement(name) => write!(
                f,
                "'{name}' is not recognized as a valid pseudo-element. Did you mean ':{name}' (pseudo-class) or is this a typo?"
            ),
    ...
    fn from(err: SelectorParseErrorKind<'i>) -> Self {
        match &err {
            ...
            SelectorParseErrorKind::PseudoElementExpectedIdent(t) => {
                SelectorError::PseudoElementExpectedIdent(t.into())
            }
            SelectorParseErrorKind::UnsupportedPseudoClass(t) => {
                SelectorError::UnsupportedPseudoClass(t.into())
            }
            SelectorParseErrorKind::UnsupportedPseudoElement(t) => {
                SelectorError::UnsupportedPseudoElement(t.into())
            }
            SelectorParseErrorKind::UnexpectedIdent(t) => SelectorError::UnexpectedIdent(t.into()),
            SelectorParseErrorKind::ExpectedNamespace(t) => {
                SelectorError::ExpectedNamespace(t.into())
            }
            SelectorParseErrorKind::ExpectedBarInAttr(t) => {
                SelectorError::ExpectedBarInAttr(t.into())
            }
            ...
            SelectorParseErrorKind::InvalidQualNameInAttr(t) => {
                SelectorError::InvalidQualNameInAttr(t.into())
            }
            ...
            SelectorParseErrorKind::AmbiguousCssModuleClass(name) => {
                SelectorError::AmbiguousCssModuleClass(name.into())
            }
...
impl fmt::Display for MinifyErrorKind {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        use MinifyErrorKind::*;
        match self {
            CircularCustomMedia { name } => {
                write!(f, "Circular custom media query {name} detected")
            }
            CustomMediaNotDefined { name } => write!(f, "Custom media query {name} is not defined"),
...
impl fmt::Display for PrinterErrorKind {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        ...
        match self {
            AmbiguousUrlInCustomProperty { url } => write!(
                f,
                "Ambiguous url('{url}') in custom property. Relative paths are resolved from the location the var() is used, not where the custom property is defined. Use an absolute URL instead"
            ),
            FmtError => write!(f, "Printer error"),
            InvalidComposesNesting => write!(
                f,
                "The `composes` property cannot be used within nested rules"
            ),
            InvalidComposesSelector => write!(
                f,
                "The `composes` property cannot be used with a simple class selector"
            ),
            InvalidCssModulesPatternInGrid => write!(
                f,
                "The CSS modules `pattern` config must end with `[local]` for use in CSS grid line names."
            ),
        }
    }
}
