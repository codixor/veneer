...
pub fn derive_to_css(input: TokenStream) -> TokenStream {
    ...
    let imp = match &data {
        ...
        _ => {
            return syn::Error::new_spanned(ident, "ToCss derive only supports enums")
                .to_compile_error()
                .into();
        }
    };
            ...
            let fields = variant
                ...
                .map(|(index, field)| {
                    field
                        .ident
                        .as_ref()
                        .map_or_else(|| Ident::new(&format!("_{index}"), Span::call_site()), Ident::clone)
                })
                .collect::<Vec<_>>();
            ...
            while let Some((field, name)) = fields_iter.next() {
                writes.push(if fields.len() > 1 {
                    ...
                    if is_option(&field.ty) {
                        ...
                        let after_space =
                            if matches!(fields_iter.peek(), Some((field, _)) if !is_option(&field.ty)) {
