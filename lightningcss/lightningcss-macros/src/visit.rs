...
use quote::quote;
use syn::{
    Attribute, Data, DataEnum, DeriveInput, Field, Fields, GenericParam, Generics, Member, Token,
    Type, Visibility, parse::Parse, parse_macro_input, parse_quote,
};
...
pub fn derive_visit_children(input: TokenStream) -> TokenStream {
    ...
    let visit_types = if let Some(attr) = attrs
        .iter()
        .find(|attr| attr.path().is_ident("visit_types"))
    {
...
fn derive(
    ...
    let t = impl_generics.type_params().find(|g| g.ident == "R");
    let v = quote! { __V };
        ...
        Data::Enum(DataEnum { variants, .. }) => {
            let variants = variants
                ...
                .map(|variant| {
                    ...
                    ) in variant.fields.iter().enumerate()
                    {
                        let name = ident.as_ref().map_or_else(
                        ...
                        if visit_types.is_none()
                            && !seen_types.contains(ty)
                            && !skip_type(attrs)
                            && !skip_type(&variant.attrs)
                        {
                            seen_types.insert(ty.clone());
                            child_types.push(quote! {
                                <#ty as Visit<#lifetime, #t, #v>>::CHILD_TYPES.bits()
...
fn skip_type(attrs: &[Attribute]) -> bool {
...
impl Parse for VisitOptions {
    fn parse(input: syn::parse::ParseStream) -> syn::Result<Self> {
        let (visit, kind, comma) = if input.peek(syn::Ident) {
